...
const Navbar = ({show}) => {
    ...
    const MenuItems = menuItems.map((item) => {
        return ( 
            <li onClick={() => setToggleMenu(prevState => !prevState)} className='relative py-5 p-8 lg:px-4 w-full whitespace-nowrap text-dark-grey  text-left  after:absolute after:w-[15%] after:h-[2px] after:bottom-[-40px] after:left-[30px]  after:bg-orange after:mb-10 lg:after:w-0'>
    ...
    <div className='flex justify-between items-center py-4 px-5 lg:px-12'>
       ...
       <div className={`${show ? 'hidden' : 'flex'}`}><Button  content={"Get a free Estimate"} color={'orange'}/></div>
